...
Entities::Agua::Agua(coordenadas::vetorfloat pos, Gerenciadores::Gerenciador_grafico* pGraf, Gerenciadores::Gerenciador_colisoes* pGC, const char* caminho, Jogador* pJ):
	Obstaculo(pos,pGraf,pGC,caminho), pJogador(pJ), cooldown((float)COOLDOWN_AGUA)
{
	getAnimacao()->getCorpo()->setFillColor(sf::Color::Green);
}

Entities::Agua::Agua() : Obstaculo(), pJogador(nullptr), cooldown((float)COOLDOWN_AGUA)
{
}
